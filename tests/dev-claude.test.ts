...
if (!["plugin update mouthfeel@mouthfeel", "plugin enable mouthfeel@mouthfeel"].includes(command)) process.exit(9);
const root = process.cwd();
...
const exitCode = command.startsWith("plugin update") ? ${options.updateExitCode ?? 0} : ${options.enableExitCode ?? 0};
