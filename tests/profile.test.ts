...
test("compiles cumulative intensity cards with shared boundaries", () => {
  ...
  assert.match(compiled.cards[1], /Clear persona/);
  assert.doesNotMatch(compiled.cards[1], /Strong persona/);
  assert.match(compiled.cards[2], /Clear persona/);
  assert.match(compiled.cards[2], /Strong persona/);
  assert.match(compiled.cards[2], /Do not style code/i);
});
...
test("selects matching phrases by intensity, score, and stable limit", () => {
  ...
  const compiled = compileProfile(source, "Use sailor cadence.", [
    ...phrases,
    { text: "High intensity", useWhen: ["ordering bug", "sequence issue"], minIntensity: 2 },
  ]);
  assert.deepEqual(
    selectPhrases(compiled, 1, "The ordering bug caused a sequence issue").map((phrase) => phrase.text),
    ["Alpha", "Beta", "Charlie"],
  );
  assert.equal(selectPhrases(compiled, 1, "The ordering bug caused a sequence issue", 10).some((phrase) => phrase.text === "High intensity"), false);
  assert.equal(selectPhrases(compiled, 2, "The ordering bug caused a sequence issue", 10).some((phrase) => phrase.text === "High intensity"), true);
});
...
test("sailor intensity 1 requires the core explanation to stay in voice", async () => {
  const profiles = await loadProfiles("profiles");
  const sailor = profiles.find((profile) => profile.id === "sailor");
  assert.ok(sailor);
  assert.match(sailor.cards[1], /most prose sentences.*recognizably/i);
  assert.match(sailor.cards[1], /every prose paragraph/i);
  assert.match(sailor.cards[1], /silently rewrite/i);
  assert.match(sailor.cards[1], /ordinary assistant/i);
  assert.match(sailor.cards[1], /core explanatory sentences/i);
  assert.match(sailor.cards[1], /neutral technical prose.*nautical/i);
});

test("sailor intensity 2 escalates to storm force on top of the full voice", async () => {
  const profiles = await loadProfiles("profiles");
  const sailor = profiles.find((profile) => profile.id === "sailor");
  assert.ok(sailor);
  assert.match(sailor.cards[2], /every prose paragraph/i);
  assert.match(sailor.cards[2], /unmistakable/i);
  assert.match(sailor.cards[2], /storm force/i);
  assert.doesNotMatch(sailor.cards[1], /storm force/i);
});

test("card and reminder carry the full distribution contract at both intensities", () => {
  const compiled = compileProfile(source, "Use sailor cadence.", []);
  const full = /each entire natural-language reply/;

  assert.match(renderRuntimeCard(compiled, 1, ""), full);
  assert.match(renderRuntimeCard(compiled, 2, ""), full);

  const reminder1 = renderRuntimeReminder(compiled, 1, "Explain the index", { always: true });
  const reminder2 = renderRuntimeReminder(compiled, 2, "Explain the index", { always: true });
  assert.ok(reminder1 && full.test(reminder1));
  assert.ok(reminder2 && full.test(reminder2));
});
