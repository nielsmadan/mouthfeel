...
function stateRoot() {
  const base = process.platform === "win32" ? process.env.LOCALAPPDATA ?? join2(homedir(), "AppData", "Local") : process.env.XDG_STATE_HOME ?? join2(homedir(), ".local", "state");
  return join2(base, "mouthfeel", "opencode");
}
function createOpenCodePlugin(profiles2, options = {}) {
  return async function mouthfeel() {
    const store = new SidecarStore(options.stateRoot ?? stateRoot());
    await store.prune().catch(() => void 0);
    const profileIds = profiles2.map((profile) => profile.id);
    const pending = /* @__PURE__ */ new Map();
    const modelPrompts = /* @__PURE__ */ new Map();
    const prompts = /* @__PURE__ */ new Map();
    const suppressed = /* @__PURE__ */ new Set();
    const applyControl = async (sessionID, raw) => {
      ...
      const profile = result.effect === "profile-greeting" ? profiles2.find((candidate) => candidate.id === result.state.profileId) ?? null : null;
      const storedState = profile && result.effect === "profile-greeting" ? markStyled(result.state) : result.state;
      try {
    ...
    return {
      ...
      async event({ event }) {
        ...
        await store.delete(sessionID).catch(() => void 0);
      },
      async dispose() {
        pending.clear();
        modelPrompts.clear();
      ...
      async "experimental.chat.system.transform"(input, output) {
        ...
        await store.write(sessionID, markStyled(active)).catch(() => void 0);
      },
...
var Mouthfeel = createOpenCodePlugin(profiles);
export {
  Mouthfeel
};
